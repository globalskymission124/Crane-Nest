...
export interface HostPagesDict {
  ...
  calendar: {
    ...
    gridTitle: string; selectDayHint: string; priceOverride: string; savePrice: string; clearOverride: string; blockDay: string; unblockDay: string; baseLabel: string; daysUnit: string; selectedRange: string;
  };
...
const tw: HostPagesDict = {
  ...
  calendar: { title: "日曆 & Airbnb同步", subtitle: "與Airbnb iCal（.ics）雙向同步空房。", selectListing: "選擇房源", exportTitle: "① 匯出至Airbnb", exportDesc: "將此URL貼到Airbnb的「同步日曆 → 連接其他網站」，本站的預訂即會顯示於Airbnb。", copy: "複製", importTitle: "② 從Airbnb匯入", importDesc: "貼上Airbnb的iCal匯出URL（.ics）並儲存，再點「立即同步」匯入已預訂日期。", saveUrl: "儲存URL", syncNow: "立即同步", blocksTitle: "不可預訂日（封鎖）", start: "開始", endExclusive: "結束（不含）", addBlock: "新增手動封鎖", noBlocks: "沒有封鎖。", srcManual: "手動", srcAirbnb: "Airbnb", srcBooking: "本站預訂", copied: "已複製。", urlSaved: "已儲存iCal URL。", manualBlockNote: "房東手動封鎖", gridTitle: "價格・空房日曆", selectDayHint: "點選日期（再點一次可選範圍）以批次設定價格或封鎖。", priceOverride: "當日價格（日圓）", savePrice: "儲存價格", clearOverride: "恢復基準價", blockDay: "封鎖此日", unblockDay: "解除封鎖", baseLabel: "基準", daysUnit: "天", selectedRange: "已選" },
