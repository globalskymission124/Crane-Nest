...
| 対象 | URL | 主な機能 |
|------|-----|----------|
| **公式サイト/SEO** | `/site` | Google検索・Google AI Overview・Gemini・豆包などが理解しやすい多言語本文、FAQ、JSON-LD構造化データ、予約ページへの明確な導線 |
| **ゲスト** | `/stays` | 検索（高度フィルター/並び替え）、地図、予約、Stripe決済、お気に入り、旅程、レビュー、チャット、通報 |
| **ゲストメッセージ** | `/stays/messages` | 旅行者がホストとの会話へ戻れる受信箱 |
...
Supabase未設定のローカルプレビューでも、上記3アカウントは `demo123` でログインできます。
また `/stays` はローカルホスト上ではデモ物件を表示するため、DB未接続でもUI確認が可能です。

## SEO/AEO対策と予約ページの分離

- `/site`: 検索エンジン・AI回答エンジンが読み取りやすい公式サイト。多言語の本文、FAQ、JSON-LD、対応エリア、サービス要点をサーバーレンダリングで掲載。
- `/stays`: 実際の予約プラットフォーム。Airbnbのように直感的な検索、日付・人数指定、空室判定、予約、決済、旅程、メッセージを扱う。
- `public/llms.txt` と `sitemap.xml` は `/site` と `/stays` を中心に案内します。アプリ内にAI検索機能は置かず、外部検索で拾われやすい情報設計にしています。
