...
import { Plus, Pencil, Trash2, Eye, EyeOff, Rocket, ImagePlus, X, Star, Loader2 } from "lucide-react";
        ...
        <div className="mb-6 rounded-2xl border border-brand-200 bg-white p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              <span className="text-xs font-semibold text-slate-500">物件写真</span>
              ...
              <div
                ...
                className={`mt-1 flex cursor-pointer flex-col items-center justify-center gap-1 rounded-xl border-2 border-dashed py-6 text-sm transition ${
                  dragOver ? "border-brand-500 bg-brand-50 text-brand-600" : "border-slate-300 text-slate-500 hover:bg-slate-50"
                }`}
              >
                {uploadingPhotos ? (
                  <><Loader2 className="h-5 w-5 animate-spin" /> アップロード中…</>
                ) : (
                  <><ImagePlus className="h-5 w-5" /> ここに写真をドラッグ、またはクリックして選択（複数可）</>
                )}
              </div>
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-4">
                  {(editing.photos || []).map((url, i) => (
                    <div key={`${url}-${i}`} className="group relative aspect-square overflow-hidden rounded-lg border border-slate-200 bg-slate-100">
                      ...
                      {i === 0 && (
                        <span className="absolute left-1 top-1 flex items-center gap-0.5 rounded-full bg-slate-900/80 px-1.5 py-0.5 text-[10px] font-bold text-white">
                          <Star className="h-2.5 w-2.5 fill-white" /> カバー
                        </span>
                      )}
                      <button
                        type="button"
                      ...
                      {i !== 0 && (
                        <button
                          type="button"
                          onClick={() => makeCover(i)}
                          className="absolute inset-x-1 bottom-1 rounded-md bg-white/90 py-0.5 text-[10px] font-semibold text-slate-700 opacity-0 transition group-hover:opacity-100"
                        >
                          カバーにする
                        </button>
                      )}
              ...
              <p className="mt-1 text-[11px] text-slate-400">先頭の写真が一覧・詳細のカバーになります。ドラッグで追加、ホバーで削除・カバー変更。</p>
            </div>
