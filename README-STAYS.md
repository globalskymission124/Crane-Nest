...
| 対象 | URL | 主な機能 |
|------|-----|----------|
| **AI検索・紹介** | `/ai` | Gemini/豆包/ChatGPT等が読みやすいサービス概要、AI回答用ブリーフ、構造化データ、予約ページへの明確な導線 |
...
- `/ai`: AI/検索エンジンが読み取るための紹介ページ。サービス概要、対応地域、対象ユーザー、ゲスト/オーナー/管理者の役割分離、FAQ、JSON-LDを掲載。
- `/site`: 人間向けの公式サイト/LP。多言語の訴求とCMS画像を反映。
- `/stays`: 実際の予約プラットフォーム。検索、予約、決済、旅程、メッセージを扱う。
- `public/llms.txt` と `sitemap.xml` から `/ai` と `/stays` へ明確にリンクしています。
